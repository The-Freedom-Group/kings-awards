...
ORIGINS = "thewayin.freedom-fire.co.uk,localhost,127.0.0.1"
CHUNK = 50 * 1024 * 1024          # a multiple of 256 KiB, as tus on Stream requires
    ...
    with open(path, "rb") as f:
        while off < size:
            ...
            for attempt in range(5):
                p = requests.patch(loc, data=chunk, headers={"Tus-Resumable": "1.0.0", "Upload-Offset": str(off),
                                                              "Content-Type": "application/offset+octet-stream"})
                if p.status_code == 204: break
                print("  chunk retry", attempt + 1, p.status_code); time.sleep(3)
            else:
                sys.exit("Upload failed at %d bytes" % off)
            off = int(p.headers.get("Upload-Offset", off + len(chunk)))
...
def wait_ready(uid, tok):
    ...
    for _ in range(240):
        ...
        if res.get("readyToStream"): return res
...
def wire(key, uid, customer):
    ...
    if key == "tom":
        pat = r'(<button class="founder rv" id="founder"[^>]*?)( data-stream="[^"]*")?( aria-haspopup)'
    else:
    ...
    if key == "tom": d["founder"]["stream_id"] = uid
    else:
